...
export class ProductsSchema1784406766000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "product_purchases" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "user_id" uuid NOT NULL, "product_id" uuid NOT NULL, "expense_id" uuid, "quantity" numeric(12,3) NOT NULL DEFAULT '1', "unit_price" numeric(14,2), "total_price" numeric(14,2) GENERATED ALWAYS AS ("unit_price" * "quantity") STORED, "store" text, "purchased_on" date NOT NULL, "notes" text, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "product_purchases_unit_price_check" CHECK ("unit_price" IS NULL OR "unit_price" >= 0), CONSTRAINT "product_purchases_quantity_check" CHECK ("quantity" > 0), CONSTRAINT "PK_f28cf8bf29b23ab794dfc91f45c" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "consumption_cycles" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "user_id" uuid NOT NULL, "product_id" uuid NOT NULL, "purchase_id" uuid, "started_on" date NOT NULL, "depleted_on" date, "days_lasted" integer GENERATED ALWAYS AS ("depleted_on" - "started_on") STORED, "quantity" numeric(12,3) NOT NULL DEFAULT '1', "notes" text, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "consumption_cycles_quantity_check" CHECK ("quantity" > 0), CONSTRAINT "cycles_date_order" CHECK ("depleted_on" IS NULL OR "depleted_on" >= "started_on"), CONSTRAINT "PK_9deba3747f96ff8415c24c4767b" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TYPE "list_item_status" AS ENUM('pending', 'purchased', 'skipped')`);
        await queryRunner.query(`CREATE TABLE "shopping_list_items" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "list_id" uuid NOT NULL, "product_id" uuid, "free_text" text, "quantity" numeric(12,3) NOT NULL DEFAULT '1', "status" "list_item_status" NOT NULL DEFAULT 'pending', "auto_added" boolean NOT NULL DEFAULT false, "product_purchase_id" uuid, "position" integer NOT NULL DEFAULT '0', "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "shopping_list_items_quantity_check" CHECK ("quantity" > 0), CONSTRAINT "list_item_has_ref" CHECK ("product_id" IS NOT NULL OR "free_text" IS NOT NULL), CONSTRAINT "PK_043c112c02fdc1c39fbd619fadb" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX "idx_list_items_pending" ON "shopping_list_items" ("list_id", "position") WHERE "status" = 'pending'`);
        await queryRunner.query(`CREATE TABLE "shopping_lists" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "user_id" uuid NOT NULL, "name" text NOT NULL DEFAULT 'Shopping list', "is_archived" boolean NOT NULL DEFAULT false, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "PK_9289ace7dd5e768d65290f3f9de" PRIMARY KEY ("id"))`);
  ...
  `);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX "idx_list_items_pending"`);
        await queryRunner.query(`DROP TABLE "shopping_list_items"`);
        await queryRunner.query(`DROP TYPE "list_item_status"`);
        ...
        await queryRunner.query(`DROP INDEX "idx_product_purchases_expense"`);
