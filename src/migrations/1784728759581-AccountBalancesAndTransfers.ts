...
export class AccountBalancesAndTransfers1784728759581 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "account_transfers" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "user_id" uuid NOT NULL, "from_account_id" uuid NOT NULL, "to_account_id" uuid NOT NULL, "amount" numeric(14,2) NOT NULL, "occurred_on" date NOT NULL, "note" text, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "account_transfers_distinct_check" CHECK ("from_account_id" <> "to_account_id"), CONSTRAINT "account_transfers_amount_check" CHECK ("amount" > 0), CONSTRAINT "PK_9f867cea579976524868172e9b3" PRIMARY KEY ("id"))`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX "idx_account_transfers_to"`);
        await queryRunner.query(`DROP INDEX "idx_account_transfers_from"`);
